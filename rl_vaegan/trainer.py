...
from rl_vaegan.utils import (get_model_list, get_scheduler, load_vgg16,
...
class RL_VAEGAN(nn.Module):
    def __init__(self, hyperparameters):
        ...
        self.gen_a = VAEGen(hyperparameters['input_dim_a'],
                            hyperparameters['gen'])  # vae for style a
        self.gen_b = VAEGen(hyperparameters['input_dim_b'],
                            hyperparameters['gen'])  # vae for style b
        self.dis_a = MsImageDis(
            hyperparameters['input_dim_a'],
            hyperparameters['dis'])  # discriminator for style a
        self.dis_b = MsImageDis(
            hyperparameters['input_dim_b'],
            hyperparameters['dis'])  # discriminator for style b
        self.instancenorm = nn.InstanceNorm2d(512, affine=False)

        self.enc_weight_sharing = nn.Conv2d(256,
                                            256,
                                            kernel_size=(1, 1),
                                            stride=(1, 1),
                                            bias=False)
        self.dec_weight_sharing = nn.Conv2d(3,
                                            3,
                                            kernel_size=(1, 1),
                                            stride=(1, 1),
                                            bias=False)
        ...
        gen_params = list(self.gen_a.parameters()) + list(
            self.gen_b.parameters()) + list(
                self.enc_weight_sharing.parameters())
    ...
    def save_loss(self, snapshot_dir, iterations):
        ...
        with open(snapshot_dir + '/' + str(iterations + 1) + '_dis_loss.pkl',
                  'wb') as f:
            pickle.dump(self.loss_dis_ep, f)
        with open(snapshot_dir + '/' + str(iterations + 1) + '_gen_loss.pkl',
                  'wb') as f:
